...
public abstract class BaseCorfuAppUtils {
     ...
     *
     * @param typeToken is a TypeToken wrapping the (possibly generic) object class
     ...
     */
    protected <T extends ICorfuSMR<T>> CorfuTable<String, Integer> instantiateCorfuObject(
            TypeToken<CorfuTable<String, Integer>> typeToken, String name) {
        return getCorfuRuntime().getObjectsView()
                .build()
                .setStreamName(name)     // stream name
                .setTypeToken(typeToken)    // a TypeToken of the specified class
